...
class LogicBiconditional(LogicAbstract):
    """A logical biconditional for a KAOS goal model.
    ...
    def from_xml(root):
        ...
        """
        item = LogicBiconditional()
        if (root is not None) and (root.tag.lower() == 'biconditional'):
    ...
    def __init__(self):
        ...
        """
        super(LogicBiconditional, self).__init__()
        self._children = list()
    ...
    def evaluate(self, values, allow_shortcircuit=False, step=None, threshold=0.5):
        """Evaluates as a logical biconditional.
        ...
        if len(self._children) == 0:
            raise ValueError('No elements for biconditional.')
        if len(self._children) > 2:
            raise ValueError('Too many elements for biconditional.')
    ...
    def is_satisfied(self, start=None, end=None, allow_shortcircuit=False, threshold=0.5):
        """Evaluates satisfaction of logical biconditional, based on history.
        Note: uses fuzzy logic (triggers if clause exceeds threshold).
    ...
    def to_string(self, expanded=False):
        """Converts the logical biconditional into a string format.

        :param expanded: expands the string if true
        :return: a string representation of the logical biconditional
        """
        if expanded:
            ...
            return '(IFF {0} THEN {1})'.format(left, right)
        else:
            return '(IFF {0})'.format(' '.join([child.to_string(expanded=expanded) for child in self._children]))
